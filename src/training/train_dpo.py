...
def train() -> None:
    ...
    parser.add_argument(
        "--config", type=str, default="configs/train_config.yaml", help="Path to configuration file"
    )
    ...
    dataset = load_dpo_dataset(
        data_path=data_path, validation_split=config["data"].get("validation_split")
    )
    ...
    trainer_wrapper = DPOTrainerWrapper(model_name=model_name, config=config)

    # Setup trainer
    trainer_wrapper.setup_trainer(train_dataset=train_dataset, output_dir=output_dir)
