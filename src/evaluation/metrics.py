...
def compute_agent_compliance(responses: List[str]) -> Dict[str, float]:
    ...
    metrics = {
        ...
        "full_compliance": sum(
            validate_json_action(r)[0] and has_thought_prefix(r) for r in responses
        )
        / n,
    }
...
def evaluate_model(
    model, tokenizer, test_prompts: List[str], max_tokens: int = 256
) -> Dict[str, any]:
    """
    ...
    return {"metrics": metrics, "responses": responses, "prompts": test_prompts}
